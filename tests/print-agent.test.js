...
test("totem exibe o QR geral separado do QR individual da senha", () => {
  ...
  assert.match(html, /id="continueToIssueButton"/);
  assert.match(html, /id="backToTypeFromSectorsButton"/);
  assert.match(page, /const TOTEM_ASSET_VERSION = "2026\.08\.18\.2"/);
  assert.match(page, /`\/totem\.js\?v=\$\{TOTEM_ASSET_VERSION\}`/);
  assert.match(html, /id="totemStepIssue"/);
  assert.match(html, /id="backToSectorsButton"/);
  ...
  assert.match(html, /data-progress-step="issue">4 <small>Emissão<\/small>/);
  assert.ok(html.indexOf('id="totemStepType"') < html.indexOf('id="totemStepIssue"'));
